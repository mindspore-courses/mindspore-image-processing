...
from mindspore import dataset as F
...
def flip_back(output_flipped, matched_parts):
    ...
    output_flipped = ops.flip(output_flipped, dims=[3])
...
def get_max_preds(batch_heatmaps):
    ...
    """
    assert isinstance(
        batch_heatmaps, mindspore.Tensor), 'batch_heatmaps should be torch.Tensor'
    ...
    preds = ops.zeros((batch_size, num_joints, 2)).to(batch_heatmaps)

    preds[:, :, 0] = idx % w  # column 对应最大值的x坐标
    preds[:, :, 1] = ops.floor(idx / w)  # row 对应最大值的y坐标

    pred_mask = ops.gt(maxvals, 0.0).repeat(
...
def get_final_preds(batch_heatmaps: mindspore.Tensor,
                    trans: list = None,
        ...
        for n in range(coords.shape[0]):
            for p in range(coords.shape[1]):
                ...
                if 1 < px < heatmap_width - 1 and 1 < py < heatmap_height - 1:
                    diff = mindspore.Tensor(
                        [
                            hm[py][px + 1] - hm[py][px - 1],
                            hm[py + 1][px] - hm[py - 1][px]
                        ]
                    ).to(batch_heatmaps)
                    coords[n][p] += ops.sign(diff) * .25

    preds = coords.clone().asnumpy()

    # Transform back
    for i in range(coords.shape[0]):
        preds[i] = affine_points(preds[i], trans[i])

    return preds, maxvals.asnumpy()
...
def plot_heatmap(image, heatmap, kps, kps_weights):
    '''draw'''
    for kp_id, _ in enumerate(kps_weights):
...
class Normalize():
    ...
    def __call__(self, image, target):
        image = F.vision.Normalize(self.mean, self.std)(image)
        return image, target
...
class KeypointToHeatMap():
    ...
    def __call__(self, image, target):
        ...
        target["heatmap"] = mindspore.Tensor(heatmap, dtype=mindspore.float32)
        target["kps_weights"] = mindspore.Tensor(
            kps_weights, dtype=mindspore.float32)
