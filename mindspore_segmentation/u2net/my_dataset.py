...
class DUTSDataset():
    ...
    def __init__(self, root: str, train: bool = True, transforms=None):
        ...
        assert os.path.exists(
            self.mask_root), f"path '{self.mask_root}' does not exist."

        image_names = [p for p in os.listdir(
            self.image_root) if p.endswith(".jpg")]
        mask_names = [p for p in os.listdir(
            self.mask_root) if p.endswith(".png")]
        assert len(
            image_names) > 0, f"not find any images in {self.image_root}."
        ...
        self.images_path = [os.path.join(self.image_root, n)
