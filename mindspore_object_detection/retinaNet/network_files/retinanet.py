...
class RetinaNetClassificationHead(nn.Cell):
    """
    ...
    def __init__(self, in_channels, num_anchors, num_classes, prior_probability=0.01):
        ...
        conv = []
        for _ in range(4):
            conv.append(nn.Conv2d(in_channels, in_channels,
        ...
        for layer in self.conv.children():
            if isinstance(layer, nn.Conv2d):
                layer.weight.set_data(ms.common.initializer.initializer(
                    ms.common.initializer.Normal(), layer.weight.shape, layer.weight.dtype))
                layer.beta.set_data(ms.common.initializer.initializer(
                    "zeros", layer.beta.shape, layer.beta.dtype))

        self.cls_logits.weight.set_data(ms.common.initializer.initializer(
            ms.common.initializer.Normal(), self.cls_logits.weight.shape, self.cls_logits.weight.dtype))
        self.cls_logits.beta.set_data(ms.common.initializer.initializer(
            -math.log((1 - prior_probability) / prior_probability), self.cls_logits.beta.shape, self.cls_logits.beta.dtype))
    ...
    def compute_loss(self,
        ...
        for targets_per_img, cls_logits_per_img, matched_idxs_per_img in zip(targets, cls_logits, matched_idxs):
            ...
            # 找出所有前景目标
            foreground_idxs_per_img = ops.ge(matched_idxs_per_img, 0)  # ge: >=
            num_foreground = foreground_idxs_per_img.sum()

            # create the target classification
            gt_classes_target = ops.zeros_like(cls_logits_per_img)
            gt_classes_target[
            ...
            valid_idxs_per_img = ops.ne(
                matched_idxs_per_img, self.BETWEEN_THRESHOLDS)  # ne: !=
    ...
    def construct(self, x: Tensor) -> Tensor:
        ...
        return ops.cat(all_cls_logits, axis=1)


class RetinaNetRegressionHead(nn.Cell):
    """
    ...
    def compute_loss(self,
        ...
        for targets_per_img, bbox_regression_per_img, anchors_per_img, matched_idxs_per_img in \
            ...
            foreground_idxs_per_img = ops.nonzero(
                ops.ge(matched_idxs_per_img, 0))[0]  # ge: >=
            ...
            targets_regression = self.bbox_coder.encode_single(
                matched_gt_boxes_per_img, anchors_per_img)

            # compute the box regression loss
            losses.append(ops.l1_loss(
    ...
    def construct(self, x: List[Tensor]) -> Tensor:
        all_bbox_regression = []
        ...
        return ops.cat(all_bbox_regression, axis=1)
...
class RetinaNetHead(nn.Cell):
    ...
    def construct(self, x: List[Tensor]) -> Dict[str, Tensor]:
        return {
...
class RetinaNet(nn.Cell):
    ...
    Args:
        ...
        anchor_generator (AnchorGenerator): Cell that generates the anchors for a set of feature
            maps.
        head (nn.Cell): Cell run on top of the feature pyramid.
            Defaults to a Cell containing a classification and regression Cell.
    ...
    def compute_loss(self, targets, head_outputs, anchors):
        ...
        for anchors_per_img, targets_per_img in zip(anchors, targets):
            if targets_per_img["boxes"].numel() == 0:
                matched_idxs.append(ops.full(
                    (anchors_per_img.size(0),), -1, dtype=ms.int64))
                continue

            match_quality_matrix = box_ops.box_iou(
                targets_per_img["boxes"], anchors_per_img)
    ...
    def postprocess_detections(self, head_output, anchors, image_shapes):
        ...
        for index in range(num_img):
            ...
            for box_regression_per_level, logits_per_level, anchors_per_level in \
                ...
                # 移除低概率的目标
                scores_per_level = ops.sigmoid(logits_per_level).flatten()
                keep_idxs = ops.gt(
                    scores_per_level, self.score_thresh)  # gt: >
                scores_per_level = scores_per_level[keep_idxs]
                topk_idxs = ops.nonzero(keep_idxs)[0]
            ...
            img_boxes = ops.cat(img_boxes, axis=0)
            img_scores = ops.cat(img_scores, axis=0)
            img_labels = ops.cat(img_labels, axis=0)

            # non-maximum suppression
            keep = box_ops.batched_nms(
                img_boxes, img_scores, img_labels, self.nms_thresh)
    ...
    def construct(self, images, targets=None):
        ...
        if targets is not None:
            for target_idx, target in enumerate(targets):
                ...
                if degenerate_boxes.any():
                    ...
                    bb_idx = ops.nonzero(degenerate_boxes.any(dim=1))[0][0]
        ...
        if self.training:
            ...
            detections = self.transform.postprocess(
                detections, images.image_sizes, original_img_sizes)
