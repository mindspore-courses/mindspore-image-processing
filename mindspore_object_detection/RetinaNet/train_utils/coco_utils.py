...
def apply_nms(all_boxes, all_scores, thres, max_boxes):
    ...
    while order.size > 0:
        ...
        xx1 = np.maximum(x1[oi], x1[order[1:]])
        yy1 = np.maximum(y1[oi], y1[order[1:]])
        xx2 = np.minimum(x2[oi], x2[order[1:]])
        yy2 = np.minimum(y2[oi], y2[order[1:]])

        w = np.maximum(0.0, xx2 - xx1 + 1)
        h = np.maximum(0.0, yy2 - yy1 + 1)
        inter = w * h

        ovr = inter / (areas[oi] + areas[order[1:]] - inter)
...
class COCOMetrics:
    ...
    def __init__(self, anno_json, classes, num_classes, min_score, nms_threshold, max_boxes):
        ...
        self.val_cls_dict = dict((i, cls) for i, cls in enumerate(classes))
...
class InferWithDecoder(nn.Cell):
    """
Infer wrapper to decode the bbox locations."""
