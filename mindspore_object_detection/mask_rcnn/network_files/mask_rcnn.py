...
class MaskRCNNHeads(nn.SequentialCell):
    '''MaskRCNNHeads'''
    ...
    def __init__(self, in_channels, layers, dilation):
        ...
        for layer_idx, layers_features in enumerate(layers, 1):
            d[f"mask_fcn{layer_idx}"] = nn.Conv2d(next_feature,
                                                  ...
                                                  padding=dilation,
                                                  dilation=dilation)
            d[f"relu{layer_idx}"] = nn.ReLU()
            next_feature = layers_features

        super().__init__(d)
        # initial params
        for name, param in self.cells_and_names():
            if "weight" in name:
                param.weight.set_data(ms.common.initializer.initializer(
                    ...
                    param.weight.shape, param.weight.dtype))


class MaskRCNNPredictor(nn.SequentialCell):
    '''MaskRCNNPredictor'''

    def __init__(self, in_channels, dim_reduced, num_classes):
        super().__init__(OrderedDict([
            ("conv5_mask", nn.Conv2dTranspose(in_channels, dim_reduced, 2, 2, 0)),
            ("relu", nn.ReLU()),
