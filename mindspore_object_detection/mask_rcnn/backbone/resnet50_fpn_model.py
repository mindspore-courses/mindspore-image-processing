...
class Bottleneck(nn.Cell):
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None, norm_layer=None):
        ...
        self.conv1 = nn.Conv2d(in_channels=in_channel, out_channels=out_channel,
                               kernel_size=1, stride=1, has_bias=False)  # squeeze channels
        self.bn1 = norm_layer(out_channel)
        # -----------------------------------------
        self.conv2 = nn.Conv2d(in_channels=out_channel, out_channels=out_channel,
                               kernel_size=3, stride=stride, has_bias=False, padding=1)
        self.bn2 = norm_layer(out_channel)
        # -----------------------------------------
        self.conv3 = nn.Conv2d(in_channels=out_channel, out_channels=out_channel * self.expansion,
                               kernel_size=1, stride=1, has_bias=False)  # unsqueeze channels
        ...
        for m in self.cells():
            if isinstance(m, nn.Conv2d):
                m.weight.set_data(init.initializer(
                    init.HeUniform(), m.weight.shape, m.weight.dtype))
...
def resnet50_fpn_backbone(pretrain_path="",
    ...
    if pretrain_path != "":
        assert os.path.exists(
            pretrain_path), "{} is not exist.".format(pretrain_path)
        # 载入预训练权重
        print(mindspore.load_param_into_net(resnet_backbone,
              mindspore.load_checkpoint(pretrain_path, strict_load=False)))
