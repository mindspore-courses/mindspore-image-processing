...
class Backbone(nn.Cell):
    ...
    def __init__(self, pretrain_path=None):
        ...
        if pretrain_path is not None:
            ms.load_param_into_net(net, ms.load_checkpoint(pretrain_path))

        self.feature_extractor = nn.SequentialCell(*list(net.children())[:7])
...
class SSD300(nn.Cell):
    ...
    def __init__(self, backbone=None, num_classes=21):
        ...
        self.loc = nn.CellList(location_extractors)
        self.conf = nn.CellList(confidence_extractors)
        self._init_weights()
    ...
    def _build_additional_features(self, input_size):
        ...
        for i, (input_ch, output_ch, middle_ch) in enumerate(zip(input_size[:-1], input_size[1:], middle_channels)):
            ...
            layer = nn.SequentialCell(
                nn.Conv2d(input_ch, middle_ch, kernel_size=1, has_bias=False),
                nn.BatchNorm2d(middle_ch),
                nn.ReLU(),
                nn.Conv2d(middle_ch, output_ch, kernel_size=3,
                          padding=padding, stride=stride, has_bias=False),
                nn.BatchNorm2d(output_ch),
                nn.ReLU(),
            )
            additional_blocks.append(layer)
        self.additional_blocks = nn.CellList(additional_blocks)
    ...
    def _init_weights(self):
        ...
        for layer in layers:
            for param in layer.get_parameters():
                if param.dim() > 1:
                    param.weight.set_data(ms.common.initializer.initializer(
                        'xavier_uniform', param.weight.shape, param.weight.dtype))
    ...
    def bbox_view(self, features, loc_extractor, conf_extractor):
        ...
        locs, confs = ops.cat(locs, 2), ops.cat(confs, 2)
        return locs, confs

    def construct(self, image, targets=None):
        x = self.feature_extractor(image)

        # Feature Map 38x38x1024, 19x19x512, 10x10x512, 5x5x256, 3x3x256, 1x1x256
        detection_features = List[Tensor]  # [x]
...
class Loss(nn.Cell):
    ...
    def _location_vec(self, loc):
        ...
        gxy = self.scale_xy * \
            (loc[:, :2, :] - self.dboxes[:, :2, :]) / \
            self.dboxes[:, 2:, :]  # Nx2x8732
        gwh = self.scale_wh * \
            (loc[:, 2:, :] / self.dboxes[:, 2:, :]).log()  # Nx2x8732
        return ops.cat((gxy, gwh), axis=1)

    def construct(self, ploc, plabel, gloc, glabel):
        ...
        # 但不能超过总样本数8732
        neg_num = ops.clamp(3 * pos_num, max=mask.size(1)).unsqueeze(-1)
        neg_mask = ops.lt(con_rank, neg_num)  # (lt: <) Tensor [N, 8732]

        # confidence最终loss使用选取的正样本loss+选取的负样本loss
        con_loss = (con * (mask.float() + neg_mask.float())
                    ).sum(dim=1)  # Tensor [N]
        ...
        num_mask = ops.gt(pos_num, 0).float()  # 统计一个batch中的每张图像中是否存在正样本
