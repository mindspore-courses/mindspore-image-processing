...
def calc_iou_tensor(boxes1, boxes2):
    ...
    lt = ops.max(boxes1[:, None, :2], boxes2[:, :2])  # left-top [N,M,2]
    rb = ops.min(boxes1[:, None, 2:], boxes2[:, 2:])  # right-bottom [N,M,2]
...
class Encoder():
    ...
    def encode(self, bboxes_in, labels_in, criteria=0.5):
        ...
        best_dbox_ious, best_dbox_idx = ious.max(
            dim=0)  # 寻找每个default box匹配到的最大IoU
        # [nboxes,]
        _, best_bbox_idx = ious.max(dim=1)  # 寻找每个GT匹配到的最大IoU
        ...
        idx = ops.arange(0, best_bbox_idx.size(0), dtype=ms.int64)
        ...
        # [8732,]
        labels_out = ops.zeros(self.nboxes, dtype=ms.int64)
    ...
    def scale_back_batch(self, bboxes_in, scores_in):
        ...
        bboxes_in[:, :, :2] = self.scale_xy * \
            bboxes_in[:, :, :2]   # 预测的x, y回归参数
        bboxes_in[:, :, 2:] = self.scale_wh * \
            bboxes_in[:, :, 2:]   # 预测的w, h回归参数
    ...
    def decode_batch(self, bboxes_in, scores_in, criteria=0.45, max_output=200):
        ...
        for bbox, prob in zip(bboxes.split(1, 0), probs.split(1, 0)):
            bbox = bbox.squeeze(0)
            prob = prob.squeeze(0)
            outputs.append(self.decode_single_new(
    ...
    def decode_single_new(self, bboxes_in, scores_in, criteria, num_output=200):
        ...
        # [8732, 4] -> [8732, 21, 4]
        bboxes_in = bboxes_in.repeat(1, num_classes).reshape(
            scores_in.shape[0], -1, 4)

        # create labels for each prediction
        labels = ops.arange(num_classes)
        ...
        inds = ops.nonzero(scores_in > 0.05).squeeze(1)
        bboxes_in, scores_in, labels = bboxes_in[inds], scores_in[inds], labels[inds]

        # remove empty boxes
        ws, hs = bboxes_in[:, 2] - bboxes_in[:,
                                             0], bboxes_in[:, 3] - bboxes_in[:, 1]
        ...
        keep = batched_nms(bboxes_in, scores_in, labels,
                           iou_threshold=criteria)
    ...
    def decode_single(self, bboxes_in, scores_in, criteria, max_output, max_num=200):
        ...
        if not bboxes_out:  # 如果为空的话，返回空tensor，注意boxes对应的空tensor size，防止验证时出错
            return [ms.numpy.empty(shape=(0, 4)), ms.numpy.empty(shape=(0,), dtype=ms.int64), ms.numpy.empty(shape=(0,))]

        bboxes_out = ops.cat(bboxes_out, axis=0)
        scores_out = ops.cat(scores_out, axis=0)
        labels_out = Tensor(labels_out, dtype=ms.int64)
...
class DefaultBoxes():
    ...
    def __init__(self, fig_size, feat_size, steps, scales, aspect_ratios, scale_xy=0.1, scale_wh=0.2):
        ...
        self.dboxes = Tensor(
            self.default_boxes, dtype=ms.float32)  # 这里不转类型会报错
        ...
        self.dboxes_ltrb[:, 0] = self.dboxes[:, 0] - \
            0.5 * self.dboxes[:, 2]   # xmin
        self.dboxes_ltrb[:, 1] = self.dboxes[:, 1] - \
            0.5 * self.dboxes[:, 3]   # ymin
        self.dboxes_ltrb[:, 2] = self.dboxes[:, 0] + \
            0.5 * self.dboxes[:, 2]   # xmax
        self.dboxes_ltrb[:, 3] = self.dboxes[:, 1] + \
            0.5 * self.dboxes[:, 3]   # ymax
...
def dboxes300_coco():
    ...
    aspect_ratios = [[2], [2, 3], [2, 3], [2, 3],
                     [2], [2]]  # 每个预测特征层上预测的default box的ratios
...
def nms(boxes, scores, iou_threshold):
    ...
    """
    box_with_score = np.column_stack((boxes.asnumpy(), scores.asnumpy()))
    box_with_score_m = ms.Tensor(box_with_score)
    return ops.NMSWithMask(iou_threshold)(box_with_score_m)
...
class PostProcess(nn.Cell):
    ...
    def decode_single_new(self, bboxes_in, scores_in, criteria, num_output):
        ...
        # 移除低概率目标，self.scores_thresh=0.05
        inds = ops.nonzero(scores_in > 0.05).squeeze(1)
        bboxes_in, scores_in, labels = bboxes_in[inds,
                                                 :], scores_in[inds], labels[inds]
    ...
    def construct(self, bboxes_in, scores_in):
        ...
        for bbox, prob in zip(bboxes.split(1, 0), probs.split(1, 0)):  # split_size, split_dim
            ...
            prob = prob.squeeze(0)
            outputs.append(self.decode_single_new(
                bbox, prob, self.criteria, self.max_output))
        return outputs
