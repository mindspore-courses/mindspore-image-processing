...
def _resize_image_onnx(image, self_min_size, self_max_size):
    ...
    im_shape = Tensor(image)[-2:]
    min_size = ops.min(im_shape)
    max_size = ops.max(im_shape)
    scale_factor = ops.min(self_min_size / min_size,
...
def _resize_image(image, self_min_size, self_max_size):
    ...
    min_size = float(ops.min(im_shape))    # 获取高宽中的最小值
    max_size = float(ops.max(im_shape))    # 获取高宽中的最大值
...
class GeneralizedRCNNTransform(nn.Cell):
    """
    ...
    def normalize(self, image):
        """标准化处理"""
        dtype = image.dtype
        mean = Tensor(self.image_mean, dtype=dtype)
        std = Tensor(self.image_std, dtype=dtype)
    ...
    def torch_choice(self, k):
        ...
        `random.choice`
        """
        index = int(ms.numpy.empty(1).uniform_(0., float(len(k))).item())
        return k[index]
    ...
    def _onnx_batch_images(self, images, size_divisible=32):
        ...
        for i in range(images[0].dim()):
            max_size_i = ops.max(ops.stack(
                [img.shape[i] for img in images]).to(ms.float32))
            max_size.append(max_size_i)
        stride = size_divisible
        max_size[1] = (ops.ceil((max_size[1].to(ms.float32)
                                 ) / stride) * stride).to(ms.int64)
        max_size[2] = (ops.ceil((max_size[2].to(ms.float32)
                                 ) / stride) * stride).to(ms.int64)
...
def resize_boxes(boxes, original_size, new_size):
    ...
    """
    ratios = [
        Tensor(s, dtype=ms.float32) /
        Tensor(s_orig, dtype=ms.float32)
    ...
    return ops.stack((xmin, ymin, xmax, ymax), axis=1)
