...
def _onnx_get_num_anchors_and_pre_nms_top_n(ob, orig_pre_nms_top_n):
    ...
    num_anchors = Tensor(ob)[1].unsqueeze(0)
    pre_nms_top_n = F.min(F.cat(
        (Tensor([orig_pre_nms_top_n], dtype=num_anchors.dtype),
         num_anchors), 0))
...
def concat_box_prediction_layers(box_cls, box_regression):
    ...
    box_cls = F.cat(box_cls_flattened, axis=1).flatten(
        0, -2)  # start_dim, end_dim
    box_regression = F.cat(box_regression_flattened, axis=1).reshape(-1, 4)
...
class RegionProposalNetwork(nn.Cell):
    ...
    def assign_targets_to_anchors(self, anchors, targets):
        ...
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            ...
            if gt_boxes.numel() == 0:
                matched_gt_boxes_per_image = F.zeros(
                    anchors_per_image.shape, dtype=ms.float32)
                labels_per_image = F.zeros(
                    (anchors_per_image.shape[0],), dtype=ms.float32)
            else:
                ...
                labels_per_image = labels_per_image.to(dtype=ms.float32)
    ...
    def filter_proposals(self, proposals, objectness, image_shapes, num_anchors_per_level):
        ...
        levels = [F.full((n, ), idx, dtype=ms.int64)
                  for idx, n in enumerate(num_anchors_per_level)]
        levels = F.cat(levels, 0)
        ...
        image_range = F.arange(num_images)
        ...
        for boxes, scores, lvl, img_shape in zip(proposals, objectness_prob, levels, image_shapes):
            ...
            keep = F.nonzero(F.ge(scores, self.score_thresh))[
                0]  # ge: >=
            boxes, scores, lvl = boxes[keep], scores[keep], lvl[keep]
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        ...
        sampled_pos_inds = F.nonzero(
            F.cat(sampled_pos_inds, axis=0)).squeeze(1)
        sampled_neg_inds = F.nonzero(
            F.cat(sampled_neg_inds, axis=0)).squeeze(1)

        # 将所有正负样本索引拼接在一起
        sampled_inds = F.cat([sampled_pos_inds, sampled_neg_inds], axis=0)
        objectness = objectness.flatten()

        labels = F.cat(labels, axis=0)
        regression_targets = F.cat(regression_targets, axis=0)
