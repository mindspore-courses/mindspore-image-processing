...
class BalancedPositiveNegativeSampler():
    ...
    def __call__(self, matched_idxs):
        ...
        for matched_idxs_per_image in matched_idxs:
            ...
            positive = ops.nonzero(matched_idxs_per_image >= 1).squeeze(1)
            # = 0的为负样本
            negative = ops.nonzero(matched_idxs_per_image == 0).squeeze(1)
            ...
            # 随机选择指定数量的正负样本
            perm1 = ops.randperm(
                positive.numel())[:num_pos]
            perm2 = ops.randperm(
                negative.numel())[:num_neg]

            pos_idx_per_image = positive[perm1]
            neg_idx_per_image = negative[perm2]

            # create binary mask from indices
            pos_idx_per_image_mask = ops.zeros_like(
                matched_idxs_per_image, dtype=ms.uint8
            )
            neg_idx_per_image_mask = ops.zeros_like(
                matched_idxs_per_image, dtype=ms.uint8
            )
...
def encode_boxes(reference_boxes, proposals, weights):
    # type: (ms.Tensor, ms.Tensor, ms.Tensor) -> ms.Tensor
    """
...
class BoxCoder():
    ...
    def encode(self, reference_boxes, proposals):
        ...
        reference_boxes = ops.cat(reference_boxes, axis=0)
        proposals = ops.cat(proposals, axis=0)
    ...
    def encode_single(self, reference_boxes, proposals):
        ...
        weights = ms.Tensor(self.weights, dtype=dtype)
    ...
    def decode(self, rel_codes, boxes):
        ...
        assert isinstance(rel_codes, ms.Tensor)
    ...
    def decode_single(self, rel_codes, boxes):
        ...
        # limit max value, prevent sending too large values into ops.exp()
        # self.bbox_xform_clip=math.log(1000. / 16)   4.135
        dw = ops.clamp(dw, max=self.bbox_xform_clip)
        dh = ops.clamp(dh, max=self.bbox_xform_clip)
        ...
        pred_boxes1 = pred_ctr_x - \
            ms.Tensor(0.5, dtype=pred_ctr_x.dtype) * pred_w
        # ymin
        pred_boxes2 = pred_ctr_y - \
            ms.Tensor(0.5, dtype=pred_ctr_y.dtype) * pred_h
        # xmax
        ...
        pred_boxes = ops.stack(
            (pred_boxes1, pred_boxes2, pred_boxes3, pred_boxes4), axis=2).flatten(1)
        return pred_boxes
