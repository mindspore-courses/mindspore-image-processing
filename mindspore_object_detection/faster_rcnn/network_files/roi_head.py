...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    # advanced indexing
    # 返回标签类别大于0的索引
    sampled_pos_inds_subset = F.nonzero(F.gt(labels, 0)).squeeze(1)
...
class RoIHeads(nn.Cell):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):
            if gt_boxes_in_image.numel() == 0:  # 该张图像中没有gt框，为背景
                ...
                clamped_matched_idxs_in_image = F.zeros(
                    (proposals_in_image.shape[0],
                     ), dtype=ms.int64
                )
                labels_in_image = F.zeros(
                    (proposals_in_image.shape[0],
                     ), dtype=ms.int64
                )
            else:
                ...
                labels_in_image = labels_in_image.to(dtype=ms.int64)

                # label background (below the losw threshold)
    ...
    def postprocess_detections(self,
        ...
        for boxes, scores, image_shape in zip(pred_boxes_list, pred_scores_list, image_shapes):
            ...
            inds = F.nonzero(F.gt(scores, self.score_thresh)).squeeze(1)
    ...
    def construct(self,
        ...
        if targets is not None:
            for t in targets:
                floating_point_types = (ms.float_, ms.double, ms.half)
                assert t["boxes"].dtype in floating_point_types, "target boxes must of float type"
                assert t["labels"].dtype == ms.int64, "target labels must of int64 type"
        ...
        result = List[Dict[str, ms.Tensor]]
        losses = {}
        if self.training:
