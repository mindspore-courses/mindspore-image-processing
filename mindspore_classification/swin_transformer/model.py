...
class WindowPartition(nn.Cell):
    ...
    def construct(self, x: Tensor) -> Tensor:
        ...
        x = x.reshape(b * h * w // (self.window_size**2),
...
def window_partition(x, window_size):
    ...
    windows = x.transpose(0, 1, 3, 2, 4, 5).reshape(-1,
...
class DropPath(nn.Cell):
    ...
    def construct(self, x: Tensor) -> Tensor:
        ...
        random_tensor = self.dropout(ms.numpy.ones(shape))
...
class LogSpacedCPB(nn.Cell):
    ...
    def __init__(
        ...
        relative_coords_h = np.arange(
            -(self.window_size[0] - 1), self.window_size[0], dtype=float)
        relative_coords_w = np.arange(
            -(self.window_size[1] - 1), self.window_size[1], dtype=float)
        relative_coords_table = np.stack(np.meshgrid(
            relative_coords_h, relative_coords_w, indexing="ij"), axis=0)
        ...
        relative_coords_table = (
            ...
            np.log2(np.abs(relative_coords_table) + 1) / np.log2(8)
        )
        ...
        coords = np.stack(np.meshgrid(coords_h, coords_w,
                          indexing="ij"), axis=0)  # 2, Wh, Ww
        coords_flatten = coords.reshape(coords.shape[0], -1)  # 2, Wh*Ww
        relative_coords = coords_flatten[:, :, None] - \
            coords_flatten[:, None, :]  # 2, Wh*Ww, Wh*Ww
        relative_coords = np.transpose(
            relative_coords, (1, 2, 0))  # Wh*Ww, Wh*Ww, 2

        relative_coords[:, :, 0] += window_size[0] - 1  # shift to start from 0
        relative_coords[:, :, 1] += window_size[1] - 1
        relative_coords[:, :, 0] *= 2 * window_size[1] - 1

        relative_position_index = np.sum(
            relative_coords, axis=-1)  # Wh*Ww, Wh*Ww
    ...
    def construct(self) -> Tensor:
        ...
        relative_position_bias = x[ops.reshape(
        ...
        relative_position_bias = ops.expand_dims(
...
class WindowCosineAttention(nn.Cell):
    ...
    def __init__(
        ...
        # get pair-wise relative position index for each token inside the window
        self.relative_position_bias = LogSpacedCPB(
            self.window_size, num_heads, pretrained_window_size)
    ...
    def construct(self, x: Tensor, mask=None) -> Tensor:
        ...
        q = ops.reshape(
            self.q(x), (B_, N, self.num_heads, C // self.num_heads))
        q = self.Normalize(q)
        q = ops.transpose(q, (0, 2, 1, 3))

        k = ops.reshape(
            self.k(x), (B_, N, self.num_heads, C // self.num_heads))
        k = self.Normalize(k)
        k = ops.transpose(k, (0, 2, 3, 1))

        v = ops.reshape(
            self.v(x), (B_, N, self.num_heads, C // self.num_heads))
        v = ops.transpose(v, (0, 2, 1, 3))

        attn = self.matmul(q, k)

        logit_scale = ops.clip_by_value(
            self.logit_scale, clip_value_min=self.value_min, clip_value_max=self.value_max)
        ...
        if mask is not None:
            ...
            attn = ops.reshape(
                attn, (B_ // nW, nW, self.num_heads, N, N,)) + mask
        ...
        x = ops.reshape(ops.transpose(
            self.matmul(attn, v), (0, 2, 1, 3)), (B_, N, C))
        x = self.proj(x)
        x = self.proj_drop(x)
...
class Mlp(nn.Cell):
    ...
    def __init__(
        ...
        self.fc1 = nn.Dense(in_channels=in_features,
                            out_channels=hidden_features, has_bias=True)
        self.act = act_layer()
        self.fc2 = nn.Dense(in_channels=hidden_features,
                            out_channels=out_features, has_bias=True)
...
class SwinTransformerBlock(nn.Cell):
    ...
    def __init__(
        ...
        mlp_hidden_dim = int(
            (dim[0] if isinstance(dim, tuple) else dim) * mlp_ratio)
        ...
        if self.shift_size > 0:
            ...
            attn_mask = mask_windows[:, np.newaxis] - \
                mask_windows[:, :, np.newaxis]
            # [64, 49, 49]
            attn_mask = Tensor(
...
class PatchEmbed(nn.Cell):
    ...
    def __init__(
        ...
        patches_resolution = [image_size[0] //
                              patch_size[0], image_size[1] // patch_size[1]]
            ...
            if isinstance(cell, nn.Conv2d):
                ...
                cell.weight.set_data(
                    init.initializer(init.TruncatedNormal(
                        sigma=0.02), cell.weight.shape, cell.weight.dtype)
                )
                if cell.bias is not None:
                    cell.bias.set_data(init.initializer(
                        "zeros", cell.bias.shape, cell.bias.dtype))
...
def swin_base_patch4_window12_384_in22k(num_classes: int = 21841, in_channels=3, **kwargs):
    '''返回特定模型'''
    model = SwinTransformerV2(in_channels=in_channels,
                              window_size=12,
                              embed_dim=128,
                              depths=(2, 2, 18, 2),
                              num_heads=(4, 8, 16, 32),
