...
class RegHead(nn.Cell):
    def __init__(self,
        ...
        self.fc = nn.Dense(in_unit, out_unit)
    ...
    def __init__(self,
        ...
        if (in_c != out_c) or (stride != 1):
            self.downsample = ConvBNAct(
                in_c=in_c, out_c=out_c, kernel_s=1, stride=stride, act=None)
        else:
...
class RegStage(nn.Cell):
    def __init__(self,
        ...
        for i in range(depth):
            ...
            name = "b{}".format(i + 1)
            self.insert_child_to_cell(name,
...
class RegNet(nn.Cell):
    ...
    def __init__(self,
        ...
        self.stem = ConvBNAct(in_c, out_c=stem_c,
                              kernel_s=3, stride=2, padding=1)
        ...
        for i, stage_args in enumerate(stage_info):
            ...
            self.insert_child_to_cell(stage_name, RegStage(
                in_c=input_channels, **stage_args))
    ...
    def _build_stage_info(cfg: dict):
        ...
        stage_widths, stage_groups = adjust_width_groups_comp(
            stage_widths, stage_groups)
