...
class ConvLayer(nn.Cell):
    ...
    def __init__(
        ...
        block.insert_child_to_cell(child_name="conv", child_cell=conv_layer)

        if use_norm:
            norm_layer = nn.BatchNorm2d(
                num_features=out_channels, momentum=0.1)
            block.insert_child_to_cell(
                child_name="norm", child_cell=norm_layer)

        if use_act:
            act_layer = nn.SiLU()
            block.insert_child_to_cell(child_name="act", child_cell=act_layer)
...
class MobileViTBlock(nn.Cell):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.local_rep.insert_child_to_cell(
            child_name="conv_3x3", child_cell=conv_3x3_in)
        self.local_rep.insert_child_to_cell(
            child_name="conv_1x1", child_cell=conv_1x1_in)
    ...
    def unfolding(self, x: Tensor) -> Tuple[Tensor, Dict]:
        ...
        if new_w != orig_w or new_h != orig_h:
            ...
            x = mindspore.ops.interpolate(x, size=(new_h, new_w),
    ...
    def consturct(self, x: Tensor) -> Tensor:
        ...
        fm = self.fusion(mindspore.ops.cat((res, fm), axis=1))
        return fm
...
class MobileViT(nn.Cell):
    ...
    def __init__(self, model_cfg: Dict, num_classes: int = 1000):
        ...
        self.classifier.insert_child_to_cell(
            child_name="global_pool", child_cell=nn.AdaptiveAvgPool2d(1))
        self.classifier.insert_child_to_cell(
            child_name="flatten", child_cell=nn.Flatten())
        if 0.0 < model_cfg["cls_dropout"] < 1.0:
            self.classifier.insert_child_to_cell(
                child_name="dropout", child_cell=nn.Dropout(p=model_cfg["cls_dropout"]))
        self.classifier.insert_child_to_cell(child_name="fc", child_cell=nn.Dense(
            in_channels=exp_channels, out_channels=num_classes))
    ...
    def init_parameters(m):
        ...
        if isinstance(m, nn.Conv2d):
            ...
            if m.weight is not None:
                m.weight.set_data(
                    Tensor(np.ones(m.weight.data.shape, dtype="float32")))
            if m.bias is not None:
                m.bias.set_data(
                    Tensor(np.zeros(m.bias.data.shape, dtype="float32")))
            ...
            if m.weight is not None:
                m.weight.set_data(Tensor(np.random.normal(
                    0, 0.02, m.weight.data.shape).astype("float32")))
            if m.bias is not None:
                m.bias.set_data(
                    Tensor(np.zeros(m.bias.data.shape, dtype="float32")))
