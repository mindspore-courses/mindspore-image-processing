...
class TransformerEncoder(nn.Cell):
    ...
    def __init__(
        ...
        for i in range(num_layers):
            ...
            self.norm1s.append(normalization1)  # 保存层
...
class DenseHead(nn.Cell):
    ...
    def __init__(
        ...
        self.classifier = nn.Dense(
            input_channel, num_classes, has_bias=has_bias, activation=activation)
...
class MultilayerDenseHead(nn.Cell):
    ...
    def __init__(
        ...
        assert len(mid_channel) == len(activation) == len(
            keep_prob), "The length of the list should be the same."
...
def vit_b_16_224(
    ...
    return vit(image_size=image_size,
               ...
               num_layers=12,
               dropout=drop_rate,
               attention_dropout=drop_rate,  # attention-dropout
...
def vit_b_32_384(
    ...
    return vit(image_size=image_size,
               ...
               num_layers=12,
               dropout=drop_rate,
               attention_dropout=drop_rate,  # attention_dropout
               drop_path_rate=drop_path_rate,
               input_channels=in_channels,
               pool="cls",
               representation_size=768 if has_logits else None)
