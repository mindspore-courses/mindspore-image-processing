...
class MyDataLoader(DataLoader):
    ...
    def __getitem__(self, index):
        ...
        img = img.resize((min(self.image_h, self.image_w),
                         min(self.image_h, self.image_w)))
        n_x = min(self.image_h, self.image_w)
        img = img.crop((int(n_x/4), int(n_x/4), int(n_x/4*3), int(n_x/4*3)))
