...
def main():
    ...
    input_img = np.expand_dims(np.transpose(
        reshape_img, [2, 0, 1]), 0).astype(np.float32)
    ...
    # post-process
    result = non_max_suppression(mindspore.Tensor(result))[0]
    boxes = result[:, :4].numpy()
    scores = result[:, 4].numpy()
    cls = result[:, 5].numpy().astype(int)
    boxes = scale_coords(reshape_img.shape, boxes,
                         origin_img.shape, (ratio, pad))
