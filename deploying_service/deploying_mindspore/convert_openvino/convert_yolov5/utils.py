...
import numpy as np
import mindspore
from MeanAveragePrecision import MeanAveragePrecision
...
def box_iou(box1, box2):
    ...
    inter = (mindspore.ops.min(box1[:, None, 2:], box2[:, 2:]) -
             mindspore.ops.max(box1[:, None, :2], box2[:, :2])).clamp(0).prod(2)
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    t = time.time()
    output = [mindspore.ops.zeros((0, 6))
              ] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference
        # Apply constraints
        x[((x[..., 2:4] < min_wh) | (x[..., 2:4] > max_wh)).any(
            1), 4] = 0  # width-height
        ...
        if labels and len(labels[xi]):
            lb = labels[xi]
            v = mindspore.ops.zeros((len(lb), nc + 5))
            v[:, :4] = lb[:, 1:5]  # box
            v[:, 4] = 1.0  # conf
            v[range(len(lb)), lb[:, 0].long() + 5] = 1.0  # cls
            x = mindspore.ops.cat((x, v), 0)
        ...
        if multi_label:
            ...
            x = mindspore.ops.cat((box, conf, j.float()), 1)[
                conf.view(-1) > conf_thres]

        # Filter by class
        if classes is not None:
            x = x[(x[:, 5:6] == mindspore.Tensor(classes)).any(1)]
        ...
        box_with_score_m = mindspore.Tensor(box_with_score)
        _, _, selected_mask = mindspore.ops.NMSWithMask(
            iou_thres)(box_with_score_m)

        def get_true_indices(lst):
            '''返回对应列表'''
            return [i for i, val in enumerate(lst) if val]
        i = mindspore.Tensor(get_true_indices(selected_mask))  # NMS
        ...
        if merge and (1 < n < 3E3):  # Merge NMS (boxes merged using weighted mean)
            ...
            x[i, :4] = mindspore.ops.mm(weights, x[:, :4]).float(
...
class MAPMetric(Metric):
    ...
    def update(self, output, target):
        ...
        for single_target in target[0]:
            ...
            targetbox = [round(txmin), round(
                tymin), round(txmax), round(tymax)]
            targetboxes.append(targetbox)
            targetlabels.append(category)

        output = mindspore.Tensor(output[0], dtype=mindspore.float32)
        output = non_max_suppression(
            output, conf_thres=self.conf_thres, iou_thres=self.iou_thres, multi_label=True)
        ...
        preds = [
            dict(
                boxes=mindspore.Tensor(predboxes, dtype=mindspore.float32),
                labels=mindspore.Tensor(predlabels, dtype=mindspore.float32),
                scores=mindspore.Tensor(scores),
            )
        ]
        targets = [
            dict(
                boxes=mindspore.Tensor(targetboxes, dtype=mindspore.float32),
                labels=mindspore.Tensor(targetlabels, dtype=mindspore.float32),
            )
        ]
...
class MyDataLoader(DataLoader):
    ...
    def __init__(self, root, dataset="train", size=(640, 640)):
        assert dataset in [
            "train", "val"], 'dataset must be in ["train", "val"]'
